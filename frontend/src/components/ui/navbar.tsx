...
export const Navbar = ({ children, className }: NavbarProps) => {
    ...
    return (
        <motion.div
            ref={ref}
            // change fixed for not fixed nax
        ...
        >
            {items.map((item, idx) => (
                <a
                    ...
                    className="relative px-4 py-2 text-neutral-300 transition delay-100 duration-100 ease-in hover:text-blue-400"
                    key={`link-${idx}`}
                    href={item.link}
                >
                    {hovered === idx && (
    ...
    return (
        <motion.div
            ...
            className={cn(
                ...
                visible && "bg-amber-300", // "bg-neutral-950/80",
                className,
            )}
        >
            {children}
